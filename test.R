...
cliRequirement <- list(
  p0 = 0.4,    # response rate in the null hypothesis
  p1 = 0.6,    # response rate in the alternative hypothesis
...
n1Min <- 2 # minimal sample size at the first stage
nkMin <- 1  # minimal sample size at stages after the 1st one
nMaxRange <- c(10, 120) # the range of the total sample size
# Generate the constraint vector of minimal sample sizes at each stage
# This is the input of the objective function
nMinEachInterim <- c(n1Min, rep(nkMin, nStage - 1)) # (do not change)

rMin<- rep(0, nStage)
# Set the PSO configuration
algSetting <- getPSOInfo(
  nSwarm = 256,     # swarm size 
  maxIter = 800,    # number of iterations
...
optimRes  <- globpso(objFunc = kStageOptimObj, PSO_INFO = algSetting, 
                     ...
                     nMin = nMinEachInterim, rMin = rMin, cliRequirement = cliRequirement)
#
...
optimDesign <- kStageFreqCrit(
  ...
  nMax = optimRes$par[1], nMin = nMinEachInterim, rMin = rMin, cliRequirement = cliRequirement)
...
minMaxDesign <- kStageFreqCrit(
  ...
  nMax = minMaxRes$par[1], nMin = nMinEachInterim, rMin = rMin, cliRequirement = cliRequirement)
